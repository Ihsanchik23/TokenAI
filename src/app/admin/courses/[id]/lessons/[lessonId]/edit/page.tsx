...
export default async function EditLessonPage({ params }: { params: Promise<{ id: string; lessonId: string }> }) {
  ...
  return <section className="stack roomy"><div><p className="eyebrow">{data.course.title}</p><h2>Редактировать урок</h2></div><LessonEditor courseId={id} modules={data.modules} initial={initial as never} /></section>;
}
