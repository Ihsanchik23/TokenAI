...
export default async function AdminSubmissionsPage({ searchParams }: { searchParams: Promise<{ status?: string; course?: string; assignment?: string }> }) {
  ...
  if (uuidPattern.test(filters.course ?? "")) query = query.eq("assignment.lesson.module.course_id", filters.course as string);
  ...
  const filePaths = rows.flatMap((row) => row.submission_files.map((file) => file.file_path));
  const { data: signedFiles } = filePaths.length
    ? await supabase.storage.from("submission-files").createSignedUrls(filePaths, 3600)
    : { data: [] };
  const signedUrlByPath = new Map((signedFiles ?? []).map((file) => [file.path, file.signedUrl]));
  const signedRows = rows.map((row) => ({
    ...row,
    submission_files: row.submission_files.map((file) => ({ ...file, signedUrl: signedUrlByPath.get(file.file_path) ?? null })),
  }));
