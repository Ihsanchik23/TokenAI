...
export default async function LearningLessonPage({ params, searchParams }: { params: Promise<{ courseSlug: string; lessonId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  if (lesson.lesson_type === "theory") {
    ...
    content = <div className="notice"><strong>{lesson.lesson_type === "quiz" ? "Quiz Engine" : "Assignment Engine"} появится в следующей фазе.</strong><p>Этот обязательный урок нельзя отметить выполненным вручную, поэтому дальнейшая последовательность останется заблокированной.</p></div>;
  }
